var Gantt = (function () {
    ...
    class Bar {
        ...
        prepare_values() {
            ...
            if (this.gantt.options.view_mode === VIEW_MODE.HOUR) {
                this.duration = date_utils.diff(this.task._end, this.task._start, 'minute') / this.gantt.options.step;
            } else {
        ...
        compute_x() {
            ...
            let diff = date_utils.diff(task_start, gantt_start, 'hour');
            let x = (diff / step) * column_width;
            
            if (this.gantt.options.view_mode === VIEW_MODE.HOUR) {
                diff = date_utils.diff(task_start, gantt_start, 'minute');
                x = ((diff / step) * column_width) + column_width;
            }

            if (this.gantt.view_is('Month')) {
                const diff = date_utils.diff(task_start, gantt_start, 'day');
                x = (diff * column_width) / 38;
            }
            return x;
        }
    ...
    class Gantt {
        ...
        setup_gantt_dates() {
            ...
            if(this.view_is(VIEW_MODE.HOUR)) {
                this.gantt_start = date_utils.add(this.gantt_start, 'day');
        ...
        get_date_info(date, last_date, i) {
            ...
            const x_pos = {
                Hour_lower: (this.options.column_width),
                Hour_upper: 0,
        ...
        bind_bar_events() {
            ...
            $.on(document, 'scroll', e => {
                ...
                var top = 0
                if(scroll > 110){
                    var scrollGantt = (Math.max(0, -(this.$container.getBoundingClientRect().y)));
                    if(scrollGantt != 0){
                        top = scrollGantt + 120
                    } else {
                        top = scroll - 115;
                ...
                this.layers.date.setAttribute('transform', 'translate(0,'+ top +')');
            });
